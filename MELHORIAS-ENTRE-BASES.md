...
|---|---|---|
| Pastoral | Agenda com duas camadas: eventos da igreja (`eventos/{data}`, com as bases que servem) e privados por participantes (`bases/{base}/agenda`, regra genérica por base) — `apps/pastoral/src/components/agenda/` | **Por portar para as bases: mostrar `local` e `nota` do evento** no calendário/escala de cada uma (hoje só o painel os mostra). A agenda privada é genérica nas regras (`bases/{base}/agenda`) — qualquer base que um dia queira uma agenda da liderança só precisa do ecrã. O truque que vale copiar: "só algumas bases" = `dispensadaPor` com as outras, sem campo novo nem mudança nas bases. |
| Kinder, SHIFT, New | Contador de crianças no Início para QUALQUER voluntário (marca se vazio; só a líder corrige), pendente sobre o domingo mais recente até alguém marcar (`domingoDaContagem`) | Já nas três bases que têm salas. O padrão "pergunta pendente sobre o último domingo, não só sobre hoje" serve a qualquer número que alguém se esqueça de dar no próprio dia — o id do culto de domingo é a data, por isso não precisa de procurar nos eventos do mês. |
| Técnica → todas | Ordem do culto com filtro Próximos/Anteriores (abre nos próximos; num mês já passado abre nos anteriores) | Portado 2026-09 para Apoio, Backstage, Comunicação, Kinder, Louvor, Louvor Kinder, New e SHIFT (a Pessoal já tinha). Mesmo código em cada `pages/Culto.jsx` — `OrdemCultoCard` é por app, não partilhado. |
| Todas | Culto → Feedbacks mostra só os cultos que já aconteceram, do mais recente para trás, os 3 últimos + "Ver mais" (pedido 2026-09) | Feito nas dez bases de uma vez, no `pages/Culto.jsx` de cada uma. |
| Pastoral | "Presença na igreja" com a fonte escolhida POR DATA (`lib/presenca.js`, `MAPA_DESDE`): Contagem até 20/9, Mapa a partir de 27/9, sem misturar fontes dentro de um domingo nem recorrer à outra; e gráficos com a área numa caixa sem padding (`.pa-graf-area`) | Específico do painel (só ele cruza as duas fontes). A lição que vale para qualquer gráfico em HTML por cima de um SVG esticado: **tudo o que se posiciona em % tem de medir-se contra a MESMA caixa que o SVG** — um `padding` na caixa de altura fixa desalinha bolinhas e linha sem erro nenhum (bug real aqui, `LinhaTempo.jsx`). E ao trocar a fonte de um número histórico, trocar a partir de uma data, nunca para trás — os domingos antigos com a fonte nova pareciam fracos sem o serem. |
| Pastoral | A etiqueta do culto (`eventos/{e}.tipoCulto`) muda-se no Painel Pastoral com um toque, sem publicar a ordem (`definirTipoCulto`, `functions/pastoral.js`) — a ordem continua a vir da Backstage em PDF | Nada a portar: Louvor, Louvor Kinder e Backstage já leem o mesmo campo ao vivo, e mudar a etiqueta é decisão de quem publica a ordem, não de cada base. A lição que vale para qualquer base: **um seletor que só grava ao carregar num botão desligado parece funcionar e não grava nada** — se um campo tem de poder mudar sozinho, precisa do seu próprio caminho de escrita. |
| Pastoral | Exportar um relatório por impressão (`RelatorioNumeros.jsx`, Números: o período escolhido → "Guardar como PDF"), com DUAS folhas de impressão montadas na mesma app: `body[data-imprimir="numeros"]` durante a impressão decide qual sai (`exportarNumeros()`, `pastoral.css`) | Reutilizável em qualquer base que queira "exportar" sem Cloud Function nem biblioteca de PDF (Financeiro: extrato de reembolsos). A lição: **com mais de uma folha `@media print` na mesma app, cada impressão tem de dizer qual é** — senão saem todas, umas atrás das outras, sem erro nenhum. |
| Pastoral | Aviso grande "domingo sem escala", de segunda a sábado, uma vez por dia por dispositivo (`AvisoSemEscala.jsx`; `localStorage` em try/catch, o lado seguro é voltar a aparecer) | Específico do pastor (é o único que vê as dez escalas). O padrão "lembrar amanhã, não para sempre" serve a qualquer lembrete a um líder (checklist por fazer, escala por confirmar) — um aviso dispensado para sempre deixa de avisar exatamente quando ainda é preciso. |
| Pastoral | Funil com conversão real (`conversaoFunil`, `lib/contactos.js`: quem está numa etapa já passou pelas anteriores), parados há mais de 7 dias destacados com texto, e o GD escolhido ao passar para "No GD" (`contactos/{id}.gd`, `moverEtapaContacto`) | Nada a portar hoje — o funil só existe aqui. A Base Pessoal podia mostrar o GD escolhido no Formulário dela (o campo já vem no contacto que ela lê). |
| Pastoral | "Não servem há mais tempo" (Desgaste): voluntários ativos pelo último domingo servido, só das bases que escalaram alguém na janela | Serviria ao Painel do líder de qualquer base (a mesma pergunta, só para a equipa dele) — cada base já tem as escalas; é um filtro sobre `bases/{b}/pessoas` + as escalas dos últimos 2 meses. Por portar quando um líder pedir. |
