...
| 2026-09 | Pastoral | Reconstruir um `Timestamp` a partir de uma data (`AAAA-MM-DD`) + hora local de Lisboa (`timestampLisboa`/`offsetLisboaMin`, `functions/pastoral.js`) — precisa disto quem corrige um horário gravado no passado, onde `Timestamp.now()` (o padrão de sempre neste repo, ver `editarSecaoAoVivo`) não serve porque "agora" não tem nada a ver com quando o facto aconteceu. Truque: um palpite a ler a hora como UTC, corrigido pelo offset real de Lisboa nesse instante (via `Intl.DateTimeFormat`) — erra só na própria hora da troca do horário de verão, duas vezes por ano | — | Nasceu para `corrigirHoraSecaoCulto` (corrigir um momento de um culto já fechado), mas é genérico: qualquer correção de um horário passado, em qualquer base, precisa exatamente disto. Copiar direto, não reinventar — duplicar (não importar de `packages/shared`, Functions não importam de lá) é a convenção já estabelecida para pouco código puro como este. Duas ideias menores do mesmo lote, também reaproveitáveis: (1) num gráfico de linha com grelha em números fechados, a grelha deve DEFINIR o topo do gráfico (calcular as linhas primeiro, topo = uma acima do máximo), nunca o contrário — calcular o topo primeiro (`max * 1.15`) e a grelha depois pode deixar a última linha abaixo do valor máximo de verdade, bug real já visto aqui (`LinhaTempo.jsx`); (2) uma Cloud Function que precisa de escrever numa base que não é a de quem chama (`baseId` como argumento, não do token) é sempre a exceção, nunca o padrão — proteger pela capacidade elevada (`ve_tudo_pastoral`, aqui) e nunca copiar o molde de `editarVoluntario` (que lê `baseId` do token de propósito) para esse caso. |
