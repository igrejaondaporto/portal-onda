  ...
  useEffect(() => {
    ...
    entrarComTokenDaUrl().then(() => {
      parar = onAuthStateChanged(auth, async (utilizador) => {
        ...
        setACarregar(true);
        if (utilizador) {
          const { papel, baseId, veTodasEscalas, podePublicarCulto, feedbackAberto } = await meuPapel();
          setSessao({ uid: utilizador.uid, papel, baseId, veTodasEscalas, podePublicarCulto, feedbackAberto });
