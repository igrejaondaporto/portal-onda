...
export default function SheetNovoCulto({ ano, mes, onFechar, onGuardado }) {
  ...
  const [escopo, setEscopo] = useState(null); // sem padrão silencioso — o líder escolhe sempre
  ...
  async function guardar() {
    ...
    if (!escopo) return torrada("Diz se o culto é para toda a igreja ou só para a Backstage.");
  ...
  return (
    <>
      ...
      <div className="pin on" role="dialog" aria-modal="true">
        ...
        <label className="rot">Este culto é de</label>
        <div className="subtabs">
          <button data-on={escopo === "global" ? 1 : 0} onClick={() => setEscopo("global")}>Toda a igreja</button>
          <button data-on={escopo === "base" ? 1 : 0} onClick={() => setEscopo("base")}>Só a Backstage</button>
        </div>
