...
export default function SheetFuncao({ funcao, eventosDisponiveis, eventoAtual, souLiderBase = true, onFechar, onGuardado }) {
  ...
  const [horaPrevista, setHoraPrevista] = useState(funcao?.horaPrevista ?? "");
  ...
  async function guardar() {
    ...
    setAEnviar(true);
    try {
      const dados = { nome: n, descricao: descricao.trim(), fase, icone, eventoId: escopo || null, foto, horaPrevista: horaPrevista || null };
      if (funcao) {
  ...
  return (
    <>
      ...
      <div className="pin on" role="dialog" aria-modal="true">
        ...
        <label className="rot">Hora prevista (opcional)</label>
        <input className="campo" type="time" value={horaPrevista} onChange={(e) => setHoraPrevista(e.target.value)} />
        <p className="ds" style={{ marginTop: 4 }}>Se preencheres, a checklist desta fase passa a ordenar pela hora, como uma linha do tempo.</p>
        <label className="rot" style={{ marginTop: 14 }}>Símbolo</label>
