 ...
 * que o cliente não pode decidir sozinho. A escala (quem entra em cada
 * papel) também passa por função — guardarEscalaLouvor — para validar
 * o cruzamento entre bases (ver functions/index.js).
 */
import {
 ...
 *  decidir a escala com informação, nunca para bloquear ninguém. */
...
export async function obterEventosDoMes(ano, mesIndex) {
  ...
  return Promise.all(
    eventos.map(async (ev) => {
      ...
      return { ...ev, escala: esc.exists() ? { ...ESCALA_VAZIA, ...esc.data() } : ESCALA_VAZIA };
    })
  );
}
...
export function ouvirEventosDoMes(ano, mesIndex, cb) {
  ...
  const pararEventos = onSnapshot(q, (snap) => {
    ...
    const escalas = {};
    eventos.forEach((ev) => {
      escalas[ev.id] = ESCALA_VAZIA;
      pararEscalas.push(
        onSnapshot(cEscala(ev.id), (esc) => {
          escalas[ev.id] = esc.exists() ? { ...ESCALA_VAZIA, ...esc.data() } : ESCALA_VAZIA;
...
export const guardarEscala = (eventoId, { escalados, liderEscala }) =>
  chamar("guardarEscalaLouvor")({ eventoId, escalados, liderEscala }).then((r) => r.data);

/** uid → Set(domingoId) para quem está escalado/indisponível noutra
 *  base nesses domingos. Leitura pública (ver firestore.rules) — não
 ...
 *  calendário/escala desta base. Reversível. */
...
/** Não apaga — desativa. Só cultos especiais; a Cloud Function recusa domingos. */
export const excluirCultoEspecial = (eventoId) => chamar("excluirCultoEspecial")({ eventoId }).then((r) => r.data);

/** Os domingos de um ano só existem depois disto ser chamado. */
