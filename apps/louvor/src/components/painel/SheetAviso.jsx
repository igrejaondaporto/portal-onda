...
import { criarAviso, criarModelo } from "../../lib/avisos";
import { useTorrada } from "@portal/shared/lib/TorradaContext.jsx";

const DURACAO_PADRAO = 3;

/**
 * Criar aviso — com uma lista de modelos no topo (se houver) que só
 * pré-preenche o formulário, nunca publica sozinha (o líder confirma
 * sempre com "Publicar"). "Guardar como modelo" é uma opção à parte,
 * não obrigatória.
 */
export default function SheetAviso({ uid, modelos, onFechar, onGuardado }) {
  const torrada = useTorrada();
  const [texto, setTexto] = useState("");
  const [urgencia, setUrgencia] = useState("normal");
  const [duracaoDias, setDuracaoDias] = useState(String(DURACAO_PADRAO));
  ...
  async function publicar() {
    ...
    if (guardarModelo && !nomeModelo.trim()) return torrada("Dá um nome ao modelo, para o encontrares depois.");
    setAPublicar(true);
    try {
