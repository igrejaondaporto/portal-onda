...
export default function SheetPessoa({
  ...
  const [categoria, setCategoria] = useState(pessoa?.categoria ?? null);
  ...
  async function guardar() {
    ...
    try {
      if (pessoa) {
        await editarVoluntario({ pessoaId: pessoa.id, nome: n, telefone: telefone.trim(), papel, foto, categoria });
        onGuardado("Voluntário atualizado");
      } else {
        const dados = { nome: n, telefone: telefone.trim(), papel, categoria };
    ...
    <>
      ...
      <div className="pin on" role="dialog" aria-modal="true">
        ...
        <div className="kin-cats" style={{ marginBottom: 4 }}>
          {CATEGORIAS.map((c) => (
            <button
              key={c.id} type="button" className="kin-cat" style={varsCategoria(c.id)}
              data-on={categoria === c.id ? 1 : 0} onClick={() => setCategoria(c.id)}
            >
              {c.nome}
            </button>
          ))}
          <button type="button" className="kin-cat" style={varsCategoria(null)} data-on={!categoria ? 1 : 0} onClick={() => setCategoria(null)}>
            Sem sala
          </button>
        </div>
        ...
        <div className="subtabs">
          ...
          <button data-on={papel === "auxiliar" ? 1 : 0} onClick={() => setPapel("auxiliar")}>Líder de sala</button>
          <button data-on={papel === "lider_base" ? 1 : 0} onClick={() => setPapel("lider_base")}>Líder geral</button>
        </div>
        <p className="ds" style={{ marginTop: 8 }}>
          {papel === "voluntario"
            ? "Código de 4 dígitos."
            : "Código de 6 dígitos e as mesmas permissões — vê e edita as três salas."}
        </p>
