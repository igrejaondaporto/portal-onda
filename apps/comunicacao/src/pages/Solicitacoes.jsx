...
import { ouvirSolicitacoes, assumirSolicitacao, mudarStatusSolicitacao } from "../lib/solicitacoes";
 ...
 *  coluna em que está. Arrastável no rato (desktop); no telemóvel o
...
function CardSolicitacao({ s, ministerio, arrastavel, aArrastar, onArrastar, onLargar, onAbrir }) {
  return (
    ...
    >
      ...
      <p className="com-kcard-sub">{ministerio?.nome || s.baseSolicitanteId}</p>
...
export default function Solicitacoes({ uid, papel, ativo, definirCabecalho }) {
  ...
  return (
    <>
      ...
      <div className="com-kanban">
        {COLUNAS.map(([status, rotulo, cor]) => {
          const lista = porColuna(status);
          return (
            <div
              className="com-kcoluna" key={status}
              onDragOver={(e) => e.preventDefault()}
              onDrop={() => largarEm(status)}
            >
              <div className="com-kcoluna-cab">
                <span className="com-kcoluna-ponto" style={{ background: cor }} />
                <span className="com-kcoluna-nome">{rotulo}</span>
                <span className="com-kcoluna-conta">{lista.length}</span>
              </div>
              {lista.length === 0 && <div className="com-kcoluna-vaz">Nada aqui</div>}
              {lista.map((s) => (
                <CardSolicitacao
          ...
          {recusadas.map((s) => (
            <div className="linha" style={{ cursor: "pointer" }} key={s.id} onClick={() => setSheet({ tipo: "ver", solicitacao: s })}>
              <div style={{ flex: 1 }}>
                ...
                <p className="ds">{s.baseSolicitanteId} · {dataCurta(s.prazo)}</p>
              </div>
