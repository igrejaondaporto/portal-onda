...
export default function Brand({ papel, ativo, definirCabecalho }) {
  ...
  const gruposAcervo = [
    ...categorias.map((c) => ({ id: c.id, nome: c.nome, cor: c.cor, itens: acervo.filter((i) => i.categoriaId === c.id) })),
      ...
      {aba === "acervo" && (
        <div className="sect">
          ...
          {gruposAcervo.length === 0 ? (
            ...
            gruposAcervo.map((g) => (
              <GrupoAcervo
                key={g.id} nome={g.nome} itens={g.itens} cor={g.cor} souLiderBase={souLiderBase}
