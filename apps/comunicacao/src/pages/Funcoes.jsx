...
import { ouvirVoluntarios, ouvirFuncoes, ouvirEventosDoMes, ouvirMinisterios, reordenarFuncoes } from "../lib/painel";
...
export default function Funcoes({ uid, papel, eventoIdFoco, focoSeq, ativo, definirCabecalho }) {
  ...
  // sem líder de escala rotativo nesta base (ver CLAUDE.md) — só o
  // líder da base distribui as funções, em qualquer data.
  const pode = souLiderBase;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ativo, evento, funcoesCulto.length, especiais.length]);
    ...
    <>
      ...
      {pode ? (
        <div className="caixa" style={{ background: "var(--agua)", border: 0 }}>
          ...
          <p className="ds" style={{ marginTop: 4 }}>És líder da base, podes editar qualquer data. Cada função aceita mais do que uma pessoa.</p>
        </div>
      ) : (
        <div className="caixa">
          <p className="ds">{nomeLiderBase} distribui as funções de cada culto.</p>
        </div>
      )}
