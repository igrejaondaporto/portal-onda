...
export default function SheetPessoa({
  ...
  async function guardar() {
    ...
    setAEnviar(true);
    try {
      const dados = { nome: n, telefone: telefone.trim(), papel, ministerios: ministeriosPessoa, cargo: cargo.trim() || null };
      if (pessoa) {
  ...
  return (
    <>
      ...
      <div className="pin on" role="dialog" aria-modal="true">
        ...
        <label className="rot" style={{ marginTop: 14 }}>Etiqueta (opcional)</label>
        <input className="campo" value={cargo} onChange={(e) => setCargo(e.target.value)} placeholder="Ex.: Auxiliar" />
        <p className="ds" style={{ marginTop: 4 }}>Só aparece a par do nome — não muda nenhum acesso.</p>
