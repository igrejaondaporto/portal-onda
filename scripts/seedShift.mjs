...
// id namespaced ("melissa-shift"), não "melissa" cru — evita colidir
// com uma Melissa que já exista noutra base (CLAUDE.md raiz, regra 9).
const LIDER = { id: "melissa-shift", nome: "Melissa" }; // trocar/completar o nome no Painel logo no 1º acesso
...
async function main() {
  console.log("A semear a Base SHIFT…\n");

  await db.doc(`bases/${BASE}`).set({
    nome: "Base SHIFT", slug: BASE, cor: "#7B5CFF",
    horaChegada: "09:00", horaCulto: "10:30",
    local: "Casa do Povo de Vermoim, Maia", ativa: true,
  }, { merge: true });
  console.log("documento da base gravado (nome/cor/horas — ajustar com a Melissa)");
  ...
  console.log("\n── CÓDIGO PROVISÓRIO ─────────────────────────");
  console.log(`Líder da base (Melissa): PIN ${PIN_PROVISORIO} (força troca no 1º acesso)`);
